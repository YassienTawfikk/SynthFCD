...
class SharedSynth(torch.nn.Module):
    """
    Applies the same geometric deformation to the synthetic (label→image) and
    real (FLAIR + ROI) branches. IntensityTransform is intentionally bypassed
    here; it is applied downstream after FCD augmentations.
    """
    ...
    def remap_labels(self, label_map: torch.Tensor) -> torch.Tensor:
        ...
        for cls_idx, group in enumerate(self.target_labels, start=1):
            for val in group:
                if val < max_val:
                    lut[val] = cls_idx
        output = lut[label_map.long()]
        nb_classes = len(self.target_labels) + 1
        return torch.clamp(output, 0, nb_classes - 1)
    ...
    def _forward_custom(self, slab, img, lab, roi):
        ...
        # Remap FCD lesion (21) → cortex (2) for GMM synthesis, then clamp anything
        # else out-of-range to background. Lesion appearance is handled downstream
        # by FCD augmentations; cortex is the correct tissue prior here.
        slab_safe = slab.clone()
        slab_safe[slab_safe == 21] = 2
        slab_safe[(slab_safe < 0) | (slab_safe >= n_classes)] = 0

        oh_slab = (
            torch.nn.functional.one_hot(slab_safe.long().squeeze(0), num_classes=n_classes)
            .permute(3, 0, 1, 2).float()
        )
        simg, slab_oh, (rimg, rlab, rroi) = self.synth(oh_slab, coreg=[img, lab, roi])
        slab_idx = self.remap_labels(slab_oh.argmax(dim=0, keepdim=True))
        rlab = self.remap_labels(rlab)
        return simg, slab_idx, rimg, rlab, rroi
...
class FCDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'roi_t': roi_t,
            'aug_type': aug_type,
            'subject_id': os.path.basename(os.path.dirname(lp)),
